...
// F-key navigation (mobile-style single panel doesn't apply on desktop; F8 opens settings)
await pg.keyboard.press('F8')
await pg.waitForTimeout(400)
t('F8 opens settings modal', (await pg.locator('.modal', {hasText:'Settings'}).count()) === 1)
...
// deep dive + save card + library
await pg.locator('input[placeholder*="full research station"]').fill('IBM')
await pg.locator('button', { hasText: 'RESEARCH' }).click()
...
t('mobile: 7 tabs', (await mp.locator('.mtab').count()) === 7)
t('mobile: single panel layout', (await mp.locator('.single .panel').count()) === 1)
// tab through all 7 panels
for (const tab of ['RESEARCH','WIRE','SCREEN','ALERTS','CAL','DEEP DIVE','PORTFOLIO']) {
