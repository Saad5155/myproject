...
// global markets overview → sparklines, select→inline chart→deep dive
await pg.locator('button', { hasText: 'MARKETS' }).click()
await pg.waitForTimeout(500)
t('markets overview modal opens', (await pg.locator('.modal', {hasText:'Markets'}).count()) === 1)
t('markets modal shows indices', (await pg.locator('.modal', {hasText:'INDICES'}).count()) === 1)
t('index cards render sparklines', (await pg.locator('.modal .idx-card .recharts-area-area').count()) >= 1)
...
t('selecting a ticker charts it inline (modal stays open)', (await pg.locator('.modal').count()) === 1)
t('inline price chart renders in markets', (await pg.locator('.modal .chartbox .recharts-area-area').count()) >= 1)
await pg.locator('.modal button', { hasText: 'OPEN FULL DEEP DIVE' }).click()
await pg.waitForTimeout(300)
t('deep dive opens from chart (modal closes)', (await pg.locator('.modal').count()) === 0)
